...
;angular.module('angularPoint').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('src/apInputGroup.html',
    "<div class={{state.groupClass}}><div class=form-group title={{state.description}} ng-class=\"{'has-error': $form[fieldName].$invalid}\"><label>{{ state.label }}</label><div ng-include=contentUrl ng-disabled=ngDisabled></div></div></div>"
  );
